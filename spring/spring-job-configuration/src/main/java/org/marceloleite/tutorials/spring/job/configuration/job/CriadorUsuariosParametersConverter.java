...
public class CriadorUsuariosParametersConverter implements JobParametersConverter {
	...
	public JobParameters getJobParameters(Properties properties) {
		...
		String instante = Optional.ofNullable(properties)
				.map(p -> (String) p.getOrDefault(CriadorUsuariosContextoPropriedade.INSTANTE.getNomeCompleto(),
						obterValorPadraoInstante()))
				.orElse(obterValorPadraoInstante());

		jobParametersBuilder.addString(CriadorUsuariosContextoPropriedade.INSTANTE.getNome(), instante, true);
